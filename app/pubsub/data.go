...
// i.e. order, transaction, event, which are considered to be top level
// topics
//
...
func (s *SubscriptionManager) Subscribe(req *SubscriptionRequest) {
	...
	if !ok {
		...
		case "order":
			s.Consumers[req.Topic()] = NewOrderConsumer(s.Client, tmp, s.Connection, s.DB, s.ConnLock, s.TopicLock)
		case "transaction":
...
//
// If all subtopics for `order`/ `transaction`/ `event` are
...
// holding subtopics for any of `order`/ `transaction`/ `event` root topics
